...
The local path source reads an archive from the local filesystem. It shares the same format detection as the tarball URL source: any spec ending in `.tgz` or `.tar.gz` that does not start with a protocol is treated as a local file path.

Ara packages the directory into a gzip-compressed tar archive in memory and processes it through the same security scanner as all other sources. The path can be relative (`./downloads/my-pkg.tgz`) or absolute (`/tmp/build/my-pkg.tar.gz`).
