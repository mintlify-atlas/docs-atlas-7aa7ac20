...
<Steps>
  ...
  <Step title="Run ara install">
    ...
    2. **Resolve** — applies Minimum Version Selection (MVS) to produce a deterministic dependency graph.
    3. **Fetch** — downloads tarballs from the npm registry (or other sources).
    4. **Analyze** — scans every JS/TS file in every fetched package against 17+ security patterns.
    5. **Prompt** — asks for your approval on any findings (unless `--non-interactive`).
    6. **Extract** — unpacks approved packages into `node_modules` and the content-addressed store.
    7. **Lock** — writes `ara.lock` with the full resolved graph.
  ...
  <Step title="Understand the security prompts">
    ...
    🔍 Scanning some-pkg@2.1.0...
      ...
      ⚠  credential-access (high) — process.env.AWS_SECRET_KEY in lib/config.js:10
  ...
  <Step title="Run a script with ara run">
    ...
    | `restricted` | Safe syscalls only, read-only filesystem, no network |
